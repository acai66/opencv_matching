...
namespace boyan_template_matching {
	...
	PatternMatcher::PatternMatcher(const MatcherParam& param)
	{
		...
		LearnPattern(m_TemplData, matchParam_.templateImage, matchParam_.minArea);
	...
	int PatternMatcher::match(const cv::Mat& image, std::vector<MatchResult>& matchResults)
	{
		...
		if (image.empty())
			return -1;
		if ((matchParam_.templateImage.cols < image.cols && matchParam_.templateImage.rows > image.rows) || (matchParam_.templateImage.cols > image.cols && matchParam_.templateImage.rows < image.rows))
			return -2;
		if (matchParam_.templateImage.size().area() > image.size().area())
			return -3;
		...
		int iTopLayer = GetTopLayer(&matchParam_.templateImage, (int)sqrt((double)matchParam_.minArea));
		//建立金字塔
	...
	int PatternMatcher::setTemplate(const cv::Mat& templateImage)
	{
		...
		matchParam_.templateImage = templateImage.clone();
		LearnPattern(m_TemplData, matchParam_.templateImage, matchParam_.minArea);
