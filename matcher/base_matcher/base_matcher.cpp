...
namespace boyan_template_matching
{
	...
	{
		logger_ = spdlog::get("boyan_template_matching");
	...
	bool BaseMatcher::initMatcher(const MatcherParam& param)
	{
		...
		if (matchParam_.templateImage.empty() || matchParam_.templateImage.channels() != 1)
		{
			logger_->error("image empty or channels != 1.");
			return false;
		}

		matchParam_.templateImage = param.templateImage.clone();
	...
	void BaseMatcher::drawResult(const cv::Mat& frame, std::vector<MatchResult> matchResults)
	{
		if (frame.empty())
		{
