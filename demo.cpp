...
// 动态库函数指针
typedef boyan_template_matching::Matcher* (*InitMD)(const boyan_template_matching::MatcherParam);
...
int main(int argc, char** argv)
{
	...
	// 匹配器参数
	boyan_template_matching::MatcherParam param;
	param.angle = 3;
	param.iouThreshold = 0;
	param.matcherType = boyan_template_matching::ByMatcherType::PATTERN;
	param.maxCount = 1;
	param.minArea = 256;
	param.scoreThreshold = 0.5;
	param.templateImage = templateImg;
	
	// 匹配结果
	std::vector<boyan_template_matching::MatchResult> matchResults;
	...
	boyan_template_matching::Matcher* matcher = myGetMatcher(param);
